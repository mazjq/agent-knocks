...
Codex 有完整的生命周期 hook 系统，和 `notify` 是两套独立机制——AgentStatusLight 用 hook 接入，
...
而**桌面 app 和 CLI 都会读全局 `~/.codex/hooks.json`**。所以 AgentStatusLight 把 hook 写到
...
{
  "hooks": {
    "UserPromptSubmit": [ { "hooks": [ { "type": "command", "command": "\"...AgentStatusLight.exe\" --emit --agent codex --status processing", "commandWindows": "..." } ] } ],
...
让 Codex 干一轮活，然后：
```powershell
Get-Content "$env:LOCALAPPDATA\AgentStatusLight\events.log" -Tail 15
```
出现 `codex … hook=PreToolUse` 等行 = 接通 ✅

## 卸载

`uninstall.ps1` 会删掉 `~/.codex/hooks.json`（若只含 AgentStatusLight 的 hook）并清理 config.toml 里的旧块，
你的 `notify` 和其它配置保持不动。
